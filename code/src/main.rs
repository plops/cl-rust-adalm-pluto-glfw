...
fn main() {
    ...
    let wait_group_pipeline_setup = crossbeam_utils::sync::WaitGroup::new();
    ...
    let reader_thread  = b.spawn(move ||{
        ...
        crossbeam_utils::thread::scope(| scope|{
                ...
                loop {
                    ...
                    let mut b  = &mut hb.lock().unwrap();
                                        ...
                                        b.timestamp=Utc::now();
                    {
                                                println!("{} {}:{} fft_processor  tup={}  (b.timestamp-a.timestamp)={}  b.ptr[0]={}", Utc::now(), file!(), line!(), tup, (b.timestamp-a.timestamp), b.ptr[0]);
}
                    s1.send(tup).unwrap();
};
});
            ...
            loop {
                    ...
                    for  i in 0..512 {
                                ...
                                {
                                        println!("{} {}:{} sdr_reader  count={}", Utc::now(), file!(), line!(), count);
}
        ...
        crossbeam_utils::thread::scope(| scope|{
                ...
                wg.wait();
                    ...
                    let mut b  = &mut hb.lock().unwrap();
                    {
                                                println!("{} {}:{} fft_scaler  tup={}  b.timestamp={}", Utc::now(), file!(), line!(), tup, b.timestamp);
}
                    s1.send(tup).unwrap();
};
}).unwrap();
        ...
        crossbeam_utils::thread::scope(| scope|{
                        scope.builder().name("fftw_processor".into()).spawn(| _|{
                                {
                                        println!("{} {}:{} start fftw plan ", Utc::now(), file!(), line!());
}
                                                let mut plan: fftw::plan::C2CPlan64  = fftw::plan::C2CPlan::aligned(&[512], fftw::types::Sign::Forward, fftw::types::Flag::Measure).unwrap();
